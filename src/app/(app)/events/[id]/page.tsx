...
export default async function EventPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [
    ...
    supabase.from('event_location_options').select('*').eq('party_id', id).returns<EventLocationOption[]>(),
    supabase.from('event_playlist_tracks').select('*').eq('party_id', id).order('created_at'),
  ]);

  // Enrich location options with vote counts
  const optionIds = (locationOptions ?? []).map(o => o.id);
  let enrichedLocations: EventLocationOption[] = (locationOptions ?? []).map(o => ({ ...o, vote_count: 0, user_voted: false }));
  ...
  if (optionIds.length > 0) {
    ...
    enrichedLocations = (locationOptions ?? []).map(o => ({
      ...o,
