...
## 4. Recipe Bug Reports Backup

### 4a. Backup script created (backup-reports.py)

```bash
python scripts/backup-reports.py
```

Exports all `recipe_bug_reports` rows to NDJSON format in `backups/reports-YYYY-MM-DD.ndjson`.
Deduplicates on re-run using the `id` field. Requires `.env.local` with `NEXT_PUBLIC_SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY`.

---

## 5. One-time Data Seeding

### 5a. Seed fusion recipes (if not already done)
...
- [x] 4a — backup-reports.py created
- [ ] 5a — Fusion recipes seeded
